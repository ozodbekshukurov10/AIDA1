...
export default function NeuralNetwork({ mode = 'ambient', className = '', intensity = 1 }: NeuralNetworkProps) {
  ...
  useEffect(() => {
    ...
    const colors = ['#5FE8FF', '#4D7CFF', '#8B5CF6'];
    ...
    const animate = () => {
      ...
      if (mode === 'ambient') {
        ...
        radialGrad.addColorStop(0, 'rgba(95, 232, 255, 0.04)');
        radialGrad.addColorStop(0.5, 'rgba(77, 124, 255, 0.02)');
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < maxDistance) {
            ...
            ctx.strokeStyle = `rgba(95, 232, 255, ${opacity})`;
            ...
            for (let k = j + 1; k < particles.length; k++) {
              ...
              if (dist2 < maxDistance - 20 && dist3 < maxDistance - 20) {
                ...
                ctx.fillStyle = `rgba(77, 124, 255, ${triOpacity})`;
                ctx.beginPath();
                ctx.moveTo(p1.x, p1.y);
                ctx.lineTo(p2.x, p2.y);
