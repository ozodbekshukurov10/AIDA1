"""Feedback Loop, Model Fine-tuning, Knowledge Updates."""
...
FEEDBACK_DB = BASE_DIR / "aida_feedback.db"
TRAINING_DB = BASE_DIR / "aida_training.db"
...
def _init_feedback_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS feedback (
            ...
            session_id TEXT DEFAULT '', prompt TEXT DEFAULT '',
            response TEXT DEFAULT '', provider TEXT DEFAULT '',
            mode TEXT DEFAULT '', latency_ms INTEGER DEFAULT 0,
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS error_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            error_type TEXT NOT NULL, error_message TEXT DEFAULT '',
            endpoint TEXT DEFAULT '', session_id TEXT DEFAULT '',
            created_at TEXT NOT NULL
        )
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS usage_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            endpoint TEXT NOT NULL, session_id TEXT DEFAULT '',
            mode TEXT DEFAULT '', latency_ms INTEGER DEFAULT 0,
            created_at TEXT NOT NULL
        )
    """)
...
def _init_training_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS training_data (
            ...
            domain TEXT DEFAULT '', language TEXT DEFAULT 'uz',
            rating INTEGER DEFAULT 0, created_at TEXT NOT NULL
        )
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS domain_prompts (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            domain TEXT NOT NULL UNIQUE, system_prompt TEXT DEFAULT '',
...
class FeedbackLoop:
    """User ratings, performance metrics, error tracking, usage analytics."""
    ...
    def submit_rating(self, rating: int, comment: str = "", session_id: str = "",
        ...
        self._conn.execute(
            "INSERT INTO feedback (rating,comment,session_id,prompt,response,"
            "provider,mode,latency_ms,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
    ...
    def log_error(self, error_type: str, error_message: str = "",
                  endpoint: str = "", session_id: str = "") -> Dict:
        now = datetime.utcnow().isoformat()
        self._conn.execute(
            "INSERT INTO error_log (error_type,error_message,endpoint,session_id,created_at) "
            "VALUES (?,?,?,?,?)",
    ...
    def log_usage(self, endpoint: str, session_id: str = "",
                  mode: str = "", latency_ms: int = 0) -> Dict:
        now = datetime.utcnow().isoformat()
        self._conn.execute(
            "INSERT INTO usage_log (endpoint,session_id,mode,latency_ms,created_at) "
            "VALUES (?,?,?,?,?)",
    ...
    def analytics(self) -> Dict:
        total_ratings = self._conn.execute(
    ...
    def analyze(self, question: str = "") -> str:
        ...
        data = self.analytics()
        prompt = (
            f"Savol: {question or 'Foydalanuvchi statistikasini tahlil qil'}\n\n"
            f"Statistika:\n{json.dumps(data, indent=2, ensure_ascii=False)}"
        )
        return self.respond(prompt, system_prompt=(
            "Sen AIDA Feedback Loop assistantisan. "
            "Foydalanuvchi statistikasini tahlil qil va taklif ber." + UZBEK_INSTRUCTION
        ))


class ModelFineTuning:
    """Custom training data, domain tuning, performance optimization."""
    ...
    def save_training_pair(self, prompt: str, response: str, domain: str = "",
        ...
        now = datetime.utcnow().isoformat()
        self._conn.execute(
            "INSERT INTO training_data (prompt,response,domain,language,rating,created_at) "
            "VALUES (?,?,?,?,?,?)",
            (prompt, response, domain, language, rating, now),
        )
        self._conn.commit()
        tid = self._conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    ...
    def list_domains(self) -> List[Dict]:
        rows = self._conn.execute(
            "SELECT domain, updated_at FROM domain_prompts ORDER BY updated_at DESC"
        ).fetchall()
    ...
    def analyze(self, question: str = "") -> str:
        ...
        q = question or "Modelni yaxshilash boyicha tavsiya ber"
        prompt = (
            f"Savol: {q}\n\n"
            f"Statistika:\n{json.dumps(stats, indent=2, ensure_ascii=False)}\n\n"
            f"Sozlangan domainlar:\n{json.dumps(domains, indent=2, ensure_ascii=False)}"
        )
        return self.respond(prompt, system_prompt=(
            "Sen AIDA Model Fine-tuning assistantisan. "
            "Training data va domain sozlamalarini tahlil qil." + UZBEK_INSTRUCTION
        ))


class KnowledgeUpdater:
    """New frameworks, library updates, security patches, best practices."""
    ...
    def suggest_updates(self, topic: str = "", context: str = "") -> str:
        ...
        return self.respond(prompt, system_prompt=(
            "Sen AIDA Knowledge Update assistantisan. Eng songgi framework, kutubxona, "
            "xavfsizlik va trendlar haqida malumot ber. Hozirgi yil: 2026.\n\n"
