...
class LMStudioProvider(BaseProvider):
    def __init__(self, config: ProviderConfig):
        ...
        self.base_url = config.base_url or "http://127.0.0.1:1234"
        self.client = httpx.AsyncClient(timeout=config.timeout)

    async def chat(self, messages: list[Message], **kwargs) -> Completion:
        start = time.monotonic()
        payload = {
            "model": kwargs.get("model") or self.config.model or "",
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout) as client:
            async with client.stream("POST", f"{self.base_url}/v1/chat/completions", json=payload) as resp:
                ...
                async for line in resp.aiter_lines():
                    ...
                    yield StreamingChunk(content=delta, done=done, finish_reason=finish)
